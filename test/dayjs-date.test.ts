import { formatDate, dateUtils } from '../src/dayjs-date.js';

describe('Day.js Date Formatting', () => {
  ...
  describe('Edge Cases', () => {
    ...
    test('handles dayjs objects', () => {
      const dayjsDate = formatDate.custom(testDate, 'YYYY-MM-DD');
      expect(dayjsDate).toBe('2025-09-26');
    });
  ...
  describe('Performance comparison', () => {
    test('formats dates efficiently', () => {
      const iterations = 1000;
      const start = Date.now();
      
      for (let i = 0; i < iterations; i++) {
        formatDate.shortDate(testDate);
      }
